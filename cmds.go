...
func actionStatus(c *cli.Context) error {
	...
	if len(name) == 0 || name == "all" {
		...
		res, err = req.Get(cfg.Client.ServerURL + "/api/programs/" + name + "/" + "status")
	}
	if err != nil {
		return err
	}
...
func actionRestart(c *cli.Context) (err error) {
	name := c.Args().First()
	...
	if !success {
		fmt.Println("ReStart=>Stop failed")
		return nil
	}
	err, success = programOperate("start", name)
	if err != nil {
	...
	if success {
		...
		fmt.Println("ReStart=>Start failed")
	}
	return nil
}
...
func actionStop(c *cli.Context) (err error) {
	...
	// fmt.Println("Stop failed")
	// }else {
	// fmt.Printf("Stopped,you can check it with \"gosuv status %s\"\n",name)
	// }
	// return nil
}
