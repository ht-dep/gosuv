...
type Program struct {
	...
	CustomLog  string `yaml:"custom_log" json:"custom_log"`
...
func (p *Program) RunNotification(state FSMState) {
	// LOG("----------start RunNotification----[%p]",p)
	// defer func(){
	// LOG("----------end RunNotification----[%p]",p)
	// }()
	notis := []Notifications{}
...
func (p *Process) drain() {
	GoSub(p.Output.drain)
	GoSub(p.Stdout.drain)
	GoSub(p.Stderr.drain)
}
...
func (p *Process) startCommand() {
	...
	p.SetState(Running) //SetState创建一个协程
	// p.cmd.Start()会创建三个协程
	...
	p.retryLeft = p.StartRetries
	// LOG("p.cmd=[%p],wati=[%p]",p.cmd,p.cmd.Wait)
	//下面的go协程会创建三个协程，GoFunc创建一个，p.cmd.Wait创建一个，go自身创建一个
	GoSub(func() {
		errC := GoFunc(p.cmd.Wait)
		startTime := time.Now()
		select {
		case <-errC:
			// LOG("========errC=[%v],startTime=[%v],StartSeconds=[%v],retryLeft=[%v],retryLeft=[%v]",
			// errC,time.Since(startTime),time.Duration(p.StartSeconds)*time.Second,p.retryLeft,p.StartRetries)
...
func NewProcess(pg Program) *Process {
	...
	pr := &Process{
		FSM:       NewFSM(Stopped),
		Program:   pg,
		stopC:     make(chan syscall.Signal, 1),
	...
	pr.StateChange = func(_, newStatus FSMState) {
		...
		GoSub(func() {
			pr.Program.RunNotification(newStatus)
		})
		// }
	}
